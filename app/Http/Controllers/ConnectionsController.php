...
class ConnectionsController extends Controller {
    ...
    public function index(Request $request) {
        ...
        return view("myconnections", array("connections" => $accpeted_connections->paginate(20), "pending_connections" => $pending_connections,
    ...
    public function follow($user_id) {
        $page = Page::getPersonalPage($user_id);
        $followRow = Following::query()->where("user_id", Auth::user()->id)->where("following", $page->id)->firstOrNew();
        $followRow->following = $page->id;
    ...
    public function unfollow($connection_id) {
        $followRow = Following::query()->where("user_id", Auth::user()->id)
                        ->where(function($query) use ($connection_id) {
                            $query->where("id", $connection_id)->orWhere("following", $connection_id);
                        })->firstOrFail();
