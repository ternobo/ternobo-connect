<template>
	<div class="chat-container">
		<div class="chats-list">
			...
			<div class="chats" v-if="connections.length > 0">
				<chat-item v-for="(connection, index) in connections" :key="'connections_id_' + connection.id" @click.native="createChat(connection.user.id)" :user.sync="connection.user[index]"></chat-item>
				<div class="d-flex justify-content-center w-100" v-if="connections_next_page_url != null" v-reached="loadMoreConnections">
					<loading-spinner></loading-spinner>
				</div>
			</div>
		</div>
