...
<li v-if="skill != undefined">
    <SkillCreditModal @credited="credited" v-if="$page.user != null && !checkUser(user.id)" :show.sync="creditModal" :user="user" :skill="skillVal"></SkillCreditModal>
    ...
    methods: {
        ...
        credit() {
            if (this.canCredit) {
                ...
                this.loading = true;
                axios.delete("/skills/credit/" + this.skillVal.id, {}).then((response) => {
